...
\begin{definicija}
	Neprazna množica $M$, opremljena z operacijo $\ast$, je \pojem{monoid}, če za operacijo $\ast$ na $M$ velja: \begin{enumerate}
		\item $a \ast (b \ast c) = (a\ast b) \ast c;\forall a, b, c \in M$,
		\item $\exists e\in M; a \ast e = e\ast a = a;\forall a\in M$.
	\end{enumerate} Lastnost $1.$ se imenuje \pojem{asociativnost}, lastnost $2.$ pa \pojem{obstoj enote}. 
\end{definicija}

\begin{definicija}
	\pojem{Relacija delne urejenosti} $\leq$ na množici X je binarna relacija, ki je tranzitivna in antisimetrična. Zanjo torej velja:\\
	\begin{enumerate}
...
\begin{definicija}
	Monoid $(R, \ast)$ je \pojem{delno urejen}, če je na njem definirana relacija delne urejenosti $\leq$, ki zadošča pogoju:  
...
\begin{definicija}
	Za neprazno množico $R$, ki je opremljena z notranjima binarnima operacijama $\oplus$ in $\otimes$ pravimo, da je \pojem{polkolobar}, če zanjo velja naslednje:	\begin{enumerate}
	...
	\end{enumerate}	Oznaka: $(R, \oplus, \otimes)$. \\ Polkolobar $(R, \oplus, \otimes)$ je \textit{komutativen}, če je multiplikativna operacija $\otimes$ na njem komutativna.
\end{definicija}
...
%	Naj bo $X$ neprazna množica in $S \subseteq P(X)$ neprazen nabor podmnožic množice $X$. Nabor $S$ je polkolobar (pod)množic za operaciji (disjunktne) unije in preseka, če zanj velja:
%	\begin{enumerate}
%		\item $\emptyset\in S$
...
\begin{definicija}
	Polkolobarju $(R, +, \cdot)$, za katerega je komutativen monoid $(R, +)$ delno urejen, pravimo \pojem{dioid}. Zanj poleg osnovnih zahtev torej še velja:\\
	$a \leq \acute{a} \Rightarrow a + \hat{a} \leq \acute{a} + \hat{a};~\forall a, \acute{a},\hat{a}\in R$.
\end{definicija}
...
\begin{definicija}
	Naj bo R polkolobar. $\pojem{Levi R-polmodul}$ je komutativen monoid $(M, +)$ z aditivno identiteto $\theta$, na katerem imamo definirano preslikavo: \newline \map{\cdot}{R\times M}{M}, ki jo imenujemo množenje s skalarjem. Ta preslikava zadošča naslednjim pogojem za vsaka $\lambda,\mu\in R$ in vsaka $a, b\in M$:
	\begin{enumerate}
...
\begin{opomba}
	Od zdaj naprej bomo pod imenom polmodul obravnavali leve polmodule. Analogi rezultatov, ki jih bomo dokazali, seveda veljajo v tudi za desne polmodule.
\end{opomba}
...
\begin{zgled}
	...
	\begin{align}
		a + b = ((a_1 + b_1, a_2 + b_2, \ldots, a_n + b_n)^{\top})
	\end{align}
	in
	\begin{align}
