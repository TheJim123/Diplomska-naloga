...
Izkaže se, da implikacija v drugo smer ne velja. Primer, ki to pokaže, se najde v polmodulu $(\R\cup\{-\infty\})^3$ nad tropskim polkolobarjem $(\R\cup\{-\infty\}, \max, +)$. Bolj natančno, vsaka družina vektorjev $\begin{bmatrix}x_i & 0 & -x_i\end{bmatrix}^\top$ za $i\in \{1, 2, \ldots, m\}$ je šibko linearno neodvisna za poljuben $m$ in različne $x_i$. Hkrati pa je vsaka četverica teh vektorjev linearno odvisna. Utemeljitev tega ne bomo navedli, saj potrebuje teoretično orodje, ki ni predmet te naloge, je pa jo možno najti v zgledu \cite[zgled 2.\ 14.]{bib:AkianTropSemi}. Navedemo pa lahko konkreten primer iz \cite[zgled 2.\ 14.]{bib:AkianTropSemi} za vektorje $v_i = \begin{bmatrix}i & 0 & -i\end{bmatrix}^\top; i = 1, 2, 3, 4$ lahko vidimo, da so linearno odvisni, saj velja \begin{align*}0\cdot v_2 + (-1)\cdot v_4 &= \begin{bmatrix}\max\{2+0,4-1\} &\max\{0+0,0-1\}&\max\{0-2, -4-1\} \end{bmatrix}^\top \\ &= \begin{bmatrix}3 & 0 & -2\end{bmatrix}^\top\\ &= \begin{bmatrix}\max\{1-1, 3+0\}&\max\{0-1, 0+0\}&\max\{-1-1, 0-3\}\end{bmatrix}^\top \\ &= (-1)\cdot v_1 + 0\cdot v_3\end{align*}. 
...
K seštevanju in množenju še dodamo množenje s skalarjem: $\lambda \cdot A = [\lambda \otimes a_{ij}]_{ij}$
...
Sedaj se lahko zapišemo naslednjo trditev in dokaz, oba povzeta po \cite[lema 2.\ 3.]{bib:Tanbase}.
...
\begin{dokaz}
		...
		Da dokažemo drugi del trditve, najprej upoštevamo definicijo ranga polmodula. Ker je $r(M) = r$, obstaja neka baza $\Gamma$ od $M$, da velja $\abs{\Gamma} = r$. Naj bo $\Gamma = \{\gamma_1, \ldots, \gamma_r\}$ ter naj bo $B \in M_{n\times r}(R)$ prehodna matrika iz $T$ v $\Gamma$. Poleg tega naj bo $C\in M_{r\times m}$ prehodna matrika iz $\Gamma$ v $S$. Zapis $(\gamma_1, \gamma_2, \ldots, \gamma_r) = (t_1, t_2, \ldots, t_n)*B$ vstavimo v $(s_1, s_2, \ldots, s_m) = (\gamma_1, \gamma_2, \ldots, \gamma_r)*C$ in s tem pridobimo zapis $$(s_1, s_2, \ldots, s_m) = (t_1, t_2, \ldots, t_n)*B*C$$ Označimo $B*C = \widehat{A}$ in takoj vidimo, da je $\widehat{A}$ prehodna matrika med $T$ in $S$ za katero je $\rho_S(\widehat{A}) \leq r$. Po drugi strani pa nam prvi del trditve pove, da je $r \leq \rho_S(\widehat{A})$. Sledi, da je $\rho_S(\widehat{A}) = r$ in s tem je dokazan tudi drugi del trditve.
\end{dokaz}
...
\begin{trditev}\label{trd:kardprostvsneprostbaz}
	Naj bo $R$ komutativen polkolobar in $M$ končno generiran prost $R$-polmodul. Potem za poljubno bazo $S$ polmodula $M$ in za poljubno prosto bazo $T$ polmodula $M$ velja $\abs{T}\leq\abs{S}$.
\end{trditev}
