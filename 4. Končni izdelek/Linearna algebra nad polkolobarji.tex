...
\begin{definicija}
	Naj bo $w = w_1w_2\ldots w_n$ neko zaporedje dolžine $n\in\N$. Vsoti $\sum_{\pi\in P(n)}\pi(w)$ pravimo \pojem{permutacijsko zaprtje} zaporedja $w$ in ga označimo z $\llbracket w \rrbracket$.
\end{definicija}

\subsection{Pideterminanta:}\label{subsect:pideterminant}
Za dano permutacijo $\pi$ množice z $n$ elementi lahko permutacijsko zaporedje $\bar{\pi}$ uporabimo tudi na dani $n\times n$ matriki $A$ na naslednji način: $$\bar{\pi}(A) = (1, \pi(1))(2, \pi(2))\ldots (n, \pi(n))(A) = A_{1\pi(1)}A_{2\pi(2)}\ldots A_{n\pi(n)}$$
Pri tem je $A_{ij}$ element matrike $A$, ki se nahaja v $i$-ti vrstici in $j$-tem stolpcu. 
Sedaj se spomnimo definicije determinante nad polji.
\begin{definicija}
...
\end{definicija}
Formulo iz definicije lahko skrajšamo s pomočjo uporabe permutacijskega zaporedja $\bar{\pi}$ na matriki $A$. Dodatno lahko še permutacije $\pi\in P(n)$ ločimo glede na parnost. Nova formula ima obliko $$det(A) = \sum_{\pi\in P(n)}sgn(\pi)\bar{\pi}(A) = \sum_{\pi\in P^{+}(n)}\bar{\pi}(A) - \sum_{\pi\in P^{-}(n)}\bar{\pi}(A)$$
...
\begin{definicija}
	Naj bo $R$ nek polkolobar in $A\in M_n(R)$ kvadratna matrika. Urejeni dvojici podani s predpisom $$pdt(A) = (\sum_{\substack{\pi\in P^{+}(n) \\ \sigma\in P(n)}}\sigma(\bar{\pi})(A), \sum_{\substack{\pi\in P^{-}(n) \\ \sigma\in P(n)}}\sigma(\bar{\pi})(A))$$ pravimo~\pojem{pideterminanta} matrike $A$. 
\end{definicija}
