...
Izkaže se, da ni vsaka šibko linearno neodvisna množica hkrati tudi linearno neodvisna. Primer, ki to demonstrira, se najde v polmodulu $(\R\cup\{-\infty\})^3$ nad tropskim polkolobarjem $(\R\cup\{-\infty\}, \max, +)$, kar nam pove \cite[zgled 2.\ 14.]{bib:AkianTropSemi}. Kot je tam navedeno, je vsaka družina vektorjev $[x_i, 0, -x_i]\in (\R\cup\{-\infty\})^3$~za $i = \{1, 2, \ldots, m\}$ šibko linearno neodvisna za poljuben $m$ in različne $x_i$. Po drugi strani pa lahko vidmo, da so vektorji $v_i = \begin{bmatrix}i & 0 & -i\end{bmatrix}^\top$ za $i = 1, 2, 3, 4$ linearno odvisni, saj velja \begin{align*}0\cdot v_2 + (-1)\cdot v_4 &= \begin{bmatrix}\max\{2+0,4-1\} &\max\{0+0,0-1\}&\max\{0-2, -4-1\} \end{bmatrix}^\top \\ &= \begin{bmatrix}3 & 0 & -2\end{bmatrix}^\top\\ &= \begin{bmatrix}\max\{1-1, 3+0\}&\max\{0-1, 0+0\}&\max\{-1-1, 0-3\}\end{bmatrix}^\top \\ &= (-1)\cdot v_1 + 0\cdot v_3\end{align*}.
...
\begin{definicija}
	...
	Za dano permutacijo $\pi$ s $\bar{\pi}$ označimo zaporedje $(1, \pi(1)), (2, \pi(2)), \ldots, (n, \pi(n))$. Na danem produktu $w = w_1w_2\ldots w_n$ lahko permutacijo $\pi$ uporabimo po komponentah, tako da je $\pi(w) = w_{\pi(1)}w_{\pi(2)}\ldots w_{\pi(n)}$. Enako postopamo pri končnih zaporedjih $s_1s_2\ldots s_n$ dolžine $n$.
\end{definicija}
...
\begin{definicija}
	Naj bo $R$ nek polkolobar in $X\in M_n(R)$ matrika. Matriki $X$ \pojem{asociiran usmerjen obtežen graf} je graf $G(X)$, za katerega je $V = \{1, 2, \ldots, n\}$ množica vozlišč in $E = \{(i, j)\in V(X)^2;~X_{ij} \neq 0\}$ množica povezav. Vsaki povezavi $e_{ij}\in E$ pripada utež $X_{ij}$.
\end{definicija}
...
\geslo{matrix associated weighted directed graph}{matriki asociiran obtežen usmerjen graf}
...
\geslo{tropical dioid}{tropski dioid}
\geslo{tropical semiring}{tropski polkolobar}
...
\geslo{weighted graph}{obtežen graf}
\geslo{weighted path}{obtežena pot}
