...
Enak postopek lahko uporabimo za poljubno $n\times n$ matriko $X\in M_n(R)$. Začnemo torej z $LS_0 = X^n$ in $DS_0 = Tr_s(X^n)\cdot I$ oziroma $LS_1 = \llbracket X^n \rrbracket$ ter $DS_1 = \llbracket Tr_s(X^n)\cdot I\rrbracket$. Nato izmenično na vsaki strani ">popravimo"< t.~i.~ ">odvečne"< komponente, dokler ne dosežemo, da je na desni strani $pp_X^{-}(X)$, na levi pa $pp_X^{+}(X)$.
