...
V \cite[definicija 2.\,3.\,]{bib:Tanbase} Tan definira linearno neodvisnost v polmodulih nad polkolobarji drugače. Da bo pojma, za katera se izkaže, da ne sovpadata, lažje razločevati, bomo linearno (ne)odvisnost po Tanu imenovali \pojem{šibka linearna (ne)odvisnost}. Ta se glasi takole:
...
Če je $X$ linearno neodvisna, potem pogledamo posebni primer $I_1 = \{i\}$ in $I_2 = I\setminus\{i\}$. Če za nek $x\in X$ velja $\Gen{\{x\}}\cap \Gen{X\setminus\{x\}} = \{\theta\}$, potem sledi $x\notin \Gen{X\setminus\{x\}}$. Za $\forall i \in I$ potem v posebnem primeru velja $x_i \notin \Gen{X\setminus\{x_i\}}$, kar pa je v resnici ravno pogoj iz definicije šibke linearne neodvisnosti. Seveda velja tudi, da če je množica $X$ šibko linearno odvisna, potem je tudi linearno odvisna.



%Po drugi strani, če za $X$ velja pogoj iz druge karakterizacije, vzamemo dve disjunktni podmnožici, $I_1$ in $I_2$, indeksne množice $I$. Z njima generiramo $\Gen{X_{I_1}}$ ter $\Gen{X_{I_2}}$ in upoštevamo, da lahko $I_1$ zapišemo kot končno ali pa števno neskončno unijo enojcev $\{i\}$, torej $I_1 = \Cup_{i\in I_1}i$.
